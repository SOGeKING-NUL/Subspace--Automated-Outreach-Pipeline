...
/**
 * Core pipeline logic — can be called from Express route or CLI.
 * Returns a result object with summary and details.
 */
export async function runPipeline(domain, limit = 10) {
  ...
  const rawCompanies = await searchLookalikeCompanies(domain, limit);
  ...
  if (domains.length === 0) {
    ...
    return { success: true, message: 'No lookalike domains found.', summary: { total: 0, sent: 0, failed: 0, skipped: 0 } };
  }
  ...
  try {
    ...
    if (err.response?.data?.error_code === 'NO_RESULTS') {
      ...
      return { success: true, message: 'No decision-makers found.', summary: { total: 0, sent: 0, failed: 0, skipped: 0 } };
    }
    throw err;
  }
    ...
    try {
      ...
      if (action === 'q') {
        ...
        results.push({
          ...
          company: person.company || null, linkedin: person.linkedin || null,
          status: 'skipped', error: 'Batch aborted by user.'
        });
        continue;
      }
      ...
      if (action === 'k') {
        ...
        results.push({
          name: person.name, email: person.email,
          company: person.company || null, linkedin: person.linkedin || null,
          status: 'skipped', error: 'Skipped by user in terminal.'
        });
        continue;
      }
      ...
      results.push({
        name: person.name, email: person.email,
        company: person.company || null, linkedin: person.linkedin || null,
        status: 'sent', messageId: response.messageId
      });
    } catch (error) {
  ...
  return {
    success: true,
    summary: { total: enrichedPeople.length, sent: successCount, failed: failureCount, skipped: skippedCount },
    details: results
  };
}

// --- Express Route (optional, for API usage) ---
...
router.post('/run', async (req, res) => {
  ...
  try {
    const result = await runPipeline(domain, limit || 10);
